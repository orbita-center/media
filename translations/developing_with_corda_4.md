...
Это используется в других частях Corda, когда требуется рефлексия. Другой способ, как определить имя, если вы не хотите указывать конкретную строку, показан ниже:
```kotlin
companion object {
  @JvmStatic
